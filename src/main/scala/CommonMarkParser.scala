...
class CommonMarkParser {
  ...
  def parse(src: scala.io.Source) = fromList(
    transform(LazyList from parseBlocks(LazyList.from(src.getLines)).blocks)
  )
  ...
  def parseBlocks(lines: LazyList[String]) = {
    ...
    def next(s: LazyList[String]): Unit = {
      def matching(
          ...
          prev: ContainerBlock,
          blocks: List[Block]
      ): (Block, Int, String, ContainerBlock) =
        blocks match {
            ...
            b.accept(from, text, s) match {
              ...
              case Some((newfrom, newtext)) =>
                matching(
                  newfrom,
                  newtext,
                  if (b.isInstanceOf[ContainerBlock])
                    b.asInstanceOf[ContainerBlock]
                  else prev,
                  t
                )
            }
                  ...
                  def starts(
                    ...
                    start(from, text, prev) match {
                      ...
                      case Some((st, fr, tx)) =>
                        ...
                        if (
                          !(st.isInstanceOf[ParagraphBlock] && trail.last
                            .isInstanceOf[ParagraphBlock])
                        )
                          prev.open match {
                            case None => add
                            case Some(b) =>
                              if (
                                !(st.isAppendable && b.isAppendable && st.getClass == b.getClass)
                              ) {
  ...
  case class C(text: String) extends Chr {
    ...
    override def toString =
      s"C($text, lf=$leftFlanking, rf=$rightFlanking, fbp=$followedByPunct, pbp=$precededByPunct)"
  }

  def chars(
      l: List[Char],
      buf: ListBuffer[CommonMarkAST] = new ListBuffer
  ): List[CommonMarkAST] =
    l match {
      case Nil => buf.toList
        ...
        t indexOf '>' match {
          ...
          case idx =>
            ...
            if (CommonMarkParser.uriRegex.pattern.matcher(addr).matches) {
              ...
              CommonMarkParser.emailRegex.pattern.matcher(addr).matches
            ) {
  ...
  def entities(
      l: List[CommonMarkAST],
      buf: ListBuffer[CommonMarkAST],
      appends: CommonMarkAST*
  ): List[CommonMarkAST] = {
    def parseName(
        l: List[CommonMarkAST],
        buf: StringBuilder = new StringBuilder
    ): Option[(String, List[CommonMarkAST])] =
      l match {
        case C(";") :: rest => Entities(buf.toString) map ((_, rest))
        case C(c) :: t
            if buf.isEmpty && c.head.isLetter || buf.nonEmpty && c.head.isLetterOrDigit =>
          buf ++= c
          parseName(t, buf)
        case _ => None
      }
    ...
    def parseNumeric(
      ...
      l match {
        case C(";") :: rest =>
          ...
          Some(
            ((if (ch > 0xffff || ch <= 0) 0xfffd else ch).toChar.toString, rest)
          )
  ...
  def chars2string(cs: List[CommonMarkAST]) =
    cs map (_.asInstanceOf[Chr].text) mkString
  ...
  def phase2(l: List[CommonMarkAST]): List[CommonMarkAST] = {
    ...
    class DLListInput private (
      ...
      override lazy val next =
        n.following.find(_.isInstanceOf[C]) match {
          case None =>
            new DLListInput(dllist.endSentinel, line, col + 1, groups)
      ...
      override def substring(end: Input): String = n.iteratorUntil(
        end.asInstanceOf[DLListInput].n
      ) map (_.element.asInstanceOf[Chr].text) mkString
    ...
    def punctuation(elem: CommonMarkAST) =
      elem match {
        case c: Chr =>
          ...
          ("!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~" contains ch) || punctuationCategories(
            Character.getType(ch).toByte
          )
        case _ => false
      }
    ...
    def isFollowedByPunct(end: dllist.Node) =
      end.notAfterEnd && punctuation(end.element)

    def isPrecededByPunct(start: dllist.Node) =
      !start.preceding.isBeforeStart && punctuation(start.preceding.element)

    def isFollowedByWhitespace(end: dllist.Node) =
      end.isAfterEnd || whitespace(end.element)

    def isPrecededByWhitespace(start: dllist.Node) =
      start.preceding.isBeforeStart || whitespace(start.preceding.element)
          ...
          case c @ C("*" | "_") =>
            ...
            if (
              ...
              mark(
                ...
                node,
                x => {
                  x.leftFlanking = true; x.followedByPunct = followedByPunct
                }
              )
            ...
            if (
              ...
              mark(
                ...
                node,
                x => {
                  x.rightFlanking = true; x.precededByPunct = precededByPunct
                }
              )
        ...
        opt(
          seq(
            ...
            alt(
              ...
              seq(
                ch('\''),
                capture("title", zeroOrMore(noneOf('\''))),
                ch('\'')
              ),
              seq(ch('('), capture("title", zeroOrMore(noneOf(')'))), ch(')'))
            )
    ...
    def delimiters(node: dllist.Node): Unit =
      if (node.notAfterEnd)
        node.element match {
          ...
          case c @ C("!")
              if node.following.notAfterEnd && node.following.element == C(
                "["
              ) =>
            stack += Delimiter(c.text, node, 1, false, false)
          ...
          if (n.element.active) {
            linkParser(new DLListInput(n.element.node)) match {
              case Success(rest) =>
                ...
                println(dstart, dend)
                val title = rest.groups get "title" map { case (s, e) =>
                  s substring e
                }
                ...
                n.element.node precede
                  LinkAST(
                    dstart.substring(dend),
                    title,
                    textual(
                      n.element.node.following
                        .iteratorUntil(node)
                        .toList
                        .map(_.element)
                    )
                ...
                n.reverseIterator map (_.element) filter (_.s == "[") foreach (_.active =
                  false)
                n.unlink
    ...
    def processEmphsis: Unit = {
      def processEmphsis: Unit = {
        while (
          current_position.notAfterEnd && !current_position.element.closer
        ) {
        ...
        if (!current_position.isAfterEnd) {
          ...
          while (
            opener.notBeforeStart && opener != stack_bottom &&
            opener != openers_bottom(
          ...
          if (
            opener.notBeforeStart && opener != stack_bottom && opener != openers_bottom(
              current_position.element.s
            )
          ) {
            val contents: CommonMarkAST =
              fromList(
                opener.element.node
                  .skipForward(opener.element.count - 1)
                  .following
                  .unlinkUntil(current_position.element.node)
              )
            val (remove, emphasis) =
              if (
                opener.element.count >= 2 && current_position.element.count >= 2
              )
            ...
            if (opener.element.count > remove) {
              opener.element.node.following unlinkUntil opener.element.node.following
                .skipForward(remove)
              opener.element.count -= remove
            } else {
              opener.element.node unlinkUntil opener.element.node.skipForward(
                remove
              )
              opener.unlink
            }
            ...
            if (current_position.element.count > remove) {
              current_position.element.node.following.unlinkUntil(
                current_position.element.node.following.skipForward(remove)
              )
              current_position.element.count -= remove
            } else {
              current_position.element.node unlinkUntil current_position.element.node
                .skipForward(remove)
            ...
            openers_bottom(current_position.element.s) =
  ...
  def transform(
    ...
    def blankAfter(s: Seq[Block]) =
      if (s.length < 2)
        ...
        s.sliding(2)
          .exists(a => a.head != BlankBlock && a.tail.head == BlankBlock)
    ...
    s match {
      ...
      case h #:: t =>
        h match {
          ...
          case h: HTMLBlock =>
            HTMLBlockAST(h.buf.toString) :: transform(t, loose)
          case _: BreakBlock => RuleAST :: transform(t, loose)
          case h: AHeadingBlock =>
            HeadingAST(h.level, inline(h.heading), None) :: transform(t, loose)
          case h: SHeadingBlock =>
            HeadingAST(h.level, inline(h.heading), None) :: transform(t, loose)
          case p: ParagraphBlock if loose =>
            ParagraphAST(inline(p.buf.toString)) :: transform(t, loose)
          case p: ParagraphBlock =>
            inline(p.buf.toString) :: transform(t, loose)
          ...
          case f: FencedBlock =>
            CodeBlockAST(
              f.buf.toString,
              if (f.info nonEmpty) Some(escapedString(f.info)) else None,
              None
            ) ::
              transform(t, loose)
          case q: QuoteBlock =>
            BlockquoteAST(
              fromList(transform(LazyList from q.blocks))
            ) :: transform(t, loose)
          case l: ListItemBlock =>
            val (items, rest) = t span (b =>
              b.isInstanceOf[ListItemBlock] && b
                .asInstanceOf[ListItemBlock]
                .typ == l.typ
            )
            val list = l +: items.asInstanceOf[LazyList[ListItemBlock]]
            val loose1 =
              list.init.exists(i => blankAfter(i.blocks.toSeq)) || blankAfter(
                list.last.blocks.init.toSeq
              )
            val listitems = list map (b =>
              ListItemAST(fromList(transform(LazyList from b.blocks, loose1)))
            ) toList
            val hd =
              if (l.typ.isInstanceOf[BulletList])
                ...
                OrderedListAST(
                  fromList(listitems),
                  !loose1,
                  l.typ.asInstanceOf[OrderedList].start
                )
